...
The name comes from a 2020 research paper out of Facebook AI Research, which proposed coupling a retriever to a generator so the system could look things up instead of relying on memorized parameters. The paper's version was an exotic research architecture. The version that conquered the industry after 2022 is almost embarrassingly simpler: search for relevant text, paste it into the prompt, ask the question. Everything in this dive, and every commercial "chat with your data" product you've seen, elaborates on that paste.

One more piece of context worth having: RAG is the technology behind most enterprise AI deployments you encounter. When a bank's assistant answers questions about its own products, when a support bot cites the actual help article, when an internal tool answers from a wiki, that's RAG. It won by default, because it's the only approach that handles facts which change daily, respects document-level access permissions, and can show its sources. Keep those three properties in mind; they explain several design choices below.
...
An embedding model takes a piece of text and returns a vector, a long list of numbers, produced so that texts with similar meaning land near each other. "How do I get my money back?" and "What's your refund policy?" share almost no vocabulary, yet their vectors sit close together, because the model that produced them was trained on the relationships between texts, not their spelling. Closeness is measured with cosine similarity, which you can read as "how aligned do these two directions point?" and leave the trigonometry to the library.
...
Once documents are points in space, retrieval becomes geometry: embed the question, find the nearest stored chunks, return them. The lab has you build the entire "vector store" by hand, and it's deliberately anticlimactic: a list of text-and-vector pairs and a loop that scores every one against the query. That transparency is the point. Commercial vector databases are that loop with better data structures, and meeting the naive version first inoculates you against believing the database is where the intelligence lives. It isn't. It lives in the embeddings, and in the decisions this chapter turns to next.

## 4.3 Chunking: the unglamorous decision that decides everything

You don't embed whole documents. A fifty-page manual covers dozens of topics, and a single vector can only point in one direction, an average of all of them, which matches every query weakly and none well. So documents get cut into chunks, each embedded separately, so retrieval can return the specific passage that answers the question.

How you cut turns out to matter enormously, and it's the least glamorous, most consequential decision in the pipeline. Two knobs frame the tradeoff. Size: big chunks carry context but arrive diluted and eat your context budget; small chunks are precise but may not say enough to be understood alone. Overlap: adjacent chunks share a seam of words so that an idea straddling a boundary isn't lost to both neighbors.
...
**Approximate nearest-neighbor indexes** are the scale story. Brute-force scanning is exact and, for thousands of chunks, instant; at millions of vectors it becomes the bottleneck, and production systems switch to approximate indexes (FAISS, HNSW, pgvector's index types) that cluster the space and search only promising regions, trading a little recall for a large speedup. The lab builds a toy one so you can turn the dial yourself: scanning roughly 7% of the data recovers roughly 97% of the true top ten. The caveat is a career's worth of avoided complexity in one sentence: don't reach for ANN until brute force is measurably too slow, and tune the tradeoff against your eval, never on faith.

**The store itself** is the upgrade nobody puts on the list, because it looks like plumbing rather than retrieval. A teaching index lives in a list and gets cached to a file, and a file supports exactly one operation: rebuild everything. Move the same vectors into a real database (this lab uses Postgres with the pgvector extension) and the search code barely changes, since `1 - (embedding <=> query)` is the cosine similarity the from-scratch store computes by hand. What changes is everything around the search. A corpus that hasn't changed should cost nothing to re-sync, which means storing a content hash per document and comparing before embedding; a document deleted upstream must lose its vectors, or the system keeps citing a page that no longer exists, which is the most damaging bug in the genre and the hardest to notice; a changed embedding model must be caught rather than obeyed, because vectors from two models aren't comparable and, when their widths happen to match, nothing will complain. The lab's own numbers make the case: re-syncing an unchanged corpus embeds zero chunks, and editing one document of four embeds three chunks instead of twelve. On four documents that's a rounding error, and on fifty thousand pages where one page changed it's the difference between a deploy that costs nothing and one that re-embeds the company. The caveat runs the other way from the usual one: a database isn't automatically the upgrade. For a few thousand chunks on one machine, a list and a cache file is genuinely the better engineering. What the database buys is the lifecycle, so reach for it when documents *change*, not when they're merely numerous.
...
If the problem is behavior rather than knowledge (the model knows enough but answers in the wrong format, tone, or style), that's fine-tuning's territory, Chapter 13. If the model needs to act or fetch live state, that's tools, Chapter 6. The recurring triad from [CHOOSING.md](../docs/CHOOSING.md) applies: RAG changes what the model knows right now; fine-tuning changes how it behaves by default; tools change what it can do. Most "we need to fine-tune on our docs" instincts are knowledge problems wearing the wrong label, and the common production shape is a combination: fine-tune for the house style, retrieve for the facts.

One forward reference with teeth: retrieved documents are untrusted input. A RAG system reads text written by whoever wrote the documents, and a document that contains instructions ("ignore your previous instructions and...") is read by a model that has trouble telling data from commands. That makes RAG a classic vector for indirect prompt injection, and it's Chapter 7's opening exhibit.

## 4.9 Where this chapter leaves you

The capstone is a working "chat with your documents" tool: index a folder once, cache the embeddings, retrieve, answer with checkable citations. The suggested exercise is the one that makes it real: drop in your own notes, ask something only they contain, and check the citation. A model just answered from knowledge that didn't exist when it was trained, because you put the right text in front of it.

You leave this dive with the pipeline (chunk, embed, store, retrieve, blend, rerank, ground, cite), with the two-instrument diagnostic habit, and with a healthy suspicion of "strictly better." What you don't yet have is the measurement discipline to wield all these knobs responsibly at scale. That isn't a gap in this chapter; it's the next one.
