...
Everything so far finds text. What the model does with it is a separate discipline, and it is where RAG makes or loses its reputation.
...
**The store itself** is the upgrade nobody puts on the list, because it looks like plumbing rather than retrieval. A teaching index lives in a list and gets cached to a file, and a file supports exactly one operation: rebuild everything. Move the same vectors into a real database (this lab uses Postgres with the pgvector extension) and the search code barely changes, since `1 - (embedding <=> query)` is the cosine similarity the from-scratch store computes by hand. What changes is everything around the search. A corpus that has not changed should cost nothing to re-sync, which means storing a content hash per document and comparing before embedding; a document deleted upstream must lose its vectors, or the system keeps citing a page that no longer exists, which is the most damaging bug in the genre and the hardest to notice; a changed embedding model must be caught rather than obeyed, because vectors from two models are not comparable and, when their widths happen to match, nothing will complain. The lab's own numbers make the case: re-syncing an unchanged corpus embeds zero chunks, and editing one document of four embeds three chunks instead of twelve. On four documents that is a rounding error, and on fifty thousand pages where one page changed it is the difference between a deploy that costs nothing and one that re-embeds the company. The caveat runs the other way from the usual one: a database is not automatically the upgrade. For a few thousand chunks on one machine, a list and a cache file is genuinely the better engineering. What the database buys is the lifecycle, so reach for it when documents *change*, not when they are merely numerous.
