...
through a file's contents with a post-tool hook, so the model and your logs never see
...
"process 2" resume, and a counter proves each tool runs exactly once across both. Real
...
`classify` routes each ticket to a per-category handler (**branching**, since a billing
