...
Claude never executes your code. It only requests the call.
</details>
...
They're billed as **output** tokens, so a thoughtful answer literally costs more,
...
Yes, and that's the whole point. Embeddings capture *meaning*, not word overlap, so
...
It detected the client disconnect and stopped the Claude call immediately, with no
